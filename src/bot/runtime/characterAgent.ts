...
const buildCharacterAgent = (
  ...
  const withCooldown = <
    T extends { cooldown: Cooldown; character?: CharacterSnapshot },
  >(
    actionName: string,
    action: () => ResultAsync<T, ArtifactsApiError>,
  ): ResultAsync<T, ArtifactsApiError> => {
    const waitMs =
      nextActionAt === undefined ? 0 : msUntilExpiration(nextActionAt);
    ...
    const wait =
      waitMs > 0 && nextActionAt !== undefined
        ? waitUntil(nextActionAt)
        : Promise.resolve();
  ...
  const moveTo = (mapId: number): ResultAsync<void, ArtifactsApiError> =>
    character.map_id === mapId
      ? okAsync(undefined)
      : move({ map_id: mapId }).map(() => undefined);

  const rest = () =>
    withCooldown('rest', () =>
      client.rest(name).map((response) => response.data),
    );

  const gather = () =>
    withCooldown('gather', () =>
      client.gather(name).map((response) => response.data),
    );
  ...
  const equip = (items: EquipItem[]) =>
    withCooldown('equip', () =>
      client.equip(name, items).map((response) => response.data),
    );

  const unequip = (items: UnequipItem[]) =>
    withCooldown('unequip', () =>
      client.unequip(name, items).map((response) => response.data),
    );

  const giveItems = (receiverCharacter: string, items: SimpleItem[]) =>
    withCooldown('giveItems', () =>
      client
        .giveItems(name, receiverCharacter, items)
 ...
 * previous one, then records the cooldown and character state returned by the
 * API for the next call. Both are seeded from the character's current state
 * (`GET /characters/{name}`), so a freshly created agent doesn't have to guess
 * whether it's still on cooldown or where it currently is.
 */
export const createCharacterAgent = (
  ...
  client
    .getCharacter(name)
    .map((character) => buildCharacterAgent(client, name, character.data));
