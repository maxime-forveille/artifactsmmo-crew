...
export const skillLevel = (
  ...
  switch (skill) {
    ...
    default: {
      ...
      throw new Error(
        `Unhandled gathering skill: ${JSON.stringify(exhaustiveCheck)}`,
      );
    }
  }
};
...
/**
 * `character`'s level in the crafting profession `skill` (e.g.
 * `weaponcrafting_level` for `"weaponcrafting"`).
 */
export const craftSkillLevel = (
  character: Character,
  skill: CraftSkill,
): number => {
  switch (skill) {
    ...
    default: {
      const exhaustiveCheck: never = skill;
      throw new Error(
        `Unhandled craft skill: ${JSON.stringify(exhaustiveCheck)}`,
      );
    }
  }
};
...
const highestLevel = <T extends { readonly level: number }>(
  items: readonly T[],
): T | undefined =>
 ...
 * `xpRates.ts`), among those `rates` actually has data for. Returns undefined
 * if none of `monsters` has been fought recently enough to have a rate yet, so
 * callers can fall back to a different heuristic.
 */
const highestObservedRate = (
  ...
  monsters
    ...
    .reduce<Monster | undefined>((best, candidate) => {
      ...
      return rates.get(candidate.code)! > rates.get(best.code)!
        ? candidate
        : best;
    }, undefined);

/**
 * Finds the best monster to hunt next for `character`, among monsters up to
 * their own level that `isSafeToFight` still allows: whichever has the best
 * observed XP/second rate (see `xpRates.ts`) from this character's own recent
 * fight history, or - when none of the safe candidates have been fought
 * recently enough to have a rate yet - the highest-level one, as a reasonable
 * proxy until real data accumulates. Returns `undefined` if nothing qualifies
 * (e.g. even the weakest monster available isn't safe with the character's
 * current gear) - callers should treat that as a signal to look at upgrading
 * equipment instead.
 */
export const findNextSafeMonster = (
  ...
  observedMonsterXpRatesOrEmpty(client, character.name).andThen((rates) =>
    client.getMonsters({ max_level: character.level }).map((page) => {
      const safe = page.data.filter((monster) =>
        isSafeToFight(character, monster),
      );
 ...
 * highest-level resource node at or below the character's own level in that
 * skill. Unlike hunting, there's no "safety" concept for gathering - a gather
 * action can't be lost the way a fight can, and `max_level` already keeps every
 * candidate within what the character's skill level allows - so this is just
 * the highest-level match, no extra heuristic needed. Returns `undefined` if no
 * resource for this skill exists at or below the character's level in it (e.g.
 * a fresh level-1 skill with a gap before the next resource tier).
 */
