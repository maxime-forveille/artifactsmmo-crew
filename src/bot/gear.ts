...
export const equippedItemInSlot = (character: Character, slot: EquipSlot): string | undefined => {
...
/**
 * Equipment slots `findBestCombatGear` knows how to score - the ones whose
 * gear reliably contributes combat-relevant stats (hp, resistances,
 * attack, damage bonuses, crit). `bag` (inventory_space) and `rune`/
 * artifacts/utilities (no combat stats, or not handled at all - see
 * `EQUIP_SLOT_BY_ITEM_TYPE`) need a different criterion and aren't
 * included here.
 */
...
const ITEM_TYPE_BY_EQUIP_SLOT = Object.fromEntries(
  Object.entries(EQUIP_SLOT_BY_ITEM_TYPE).map(([itemType, slot]) => [slot, itemType]),
) as Record<SupportedCombatSlot, ItemType>;
...
const gearContribution = (item: Item): Partial<Record<EquipmentEffectCode, number>> =>
  Object.fromEntries(
    (item.effects ?? [])
 ...
 * Generalizes what used to be weapon-only selection: starts from
 * `character`'s stats with their *currently* equipped item in `slot`
 * removed (so it's compared on equal footing with every other candidate,
 * not double-counted), adds each candidate's contribution back in, and
 * picks whichever yields the highest `combatMargin` against `monster` -
 * the same continuous safety-margin score `isSafeToFight` uses a
 * threshold on, so armor (hp/resistances) and weapons (attack/dmg/crit)
 * are ranked on one consistent scale instead of needing per-slot ad hoc
 * weights. Returns undefined if the catalog for this slot is empty, or if
 * every candidate would still leave the character unable to deal any
 * damage at all (`combatMargin` of exactly `0`) - equipping such an item
 * would never be worth it regardless of its other stats.
 *
 * Note: this only reasons about stats that feed into the damage model
 * (see `EQUIPMENT_EFFECT_CODES`) - non-combat effects some gear also
 * grants (`wisdom`, `prospecting`, `haste`, ...) aren't weighed at all, a
 * documented simplification like `isSafeToFight`'s ignored initiative.
 */
export const findBestCombatGear = (
  ...
  return currentItem$.andThen((currentItem) => {
    ...
    return client
      .getItems({ max_level: maxLevel, size: 100, type: ITEM_TYPE_BY_EQUIP_SLOT[slot] })
      .map((page) => {
        ...
        return ranked.reduce<{ readonly item: Item; readonly margin: number } | undefined>(
          (best, candidate) =>
            candidate.margin > 0 && (best === undefined || candidate.margin > best.margin)
              ? candidate
              : best,
          undefined,
        )?.item;
