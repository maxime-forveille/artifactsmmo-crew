...
/**
 * Wraps `fn` so that, for the lifetime of the returned function, calls with the
 * same arguments (compared via `keyFor`) only ever call `fn` once and share its
 ...
 * retries for real, rather than replaying the same failure forever.
 */
...
type TtlMemoizedAsync<Args extends readonly unknown[], T, E> = ((
  ...args: Args
) => ResultAsync<T, E>) & { clear: () => void };

/**
 * Like `memoizeAsync`, but only reuses successful calls for `ttlMs`. It is
 * intended for dynamic GET endpoints whose value can be slightly stale but must
 * not be fetched every task cycle. `clear` is for callers that know an action
 * has changed the remote state before the TTL elapses.
 */
export const memoizeAsyncWithTtl = <Args extends readonly unknown[], T, E>(
  ...
  const cache = new Map<
    string,
    { expiresAt: number; result: ResultAsync<T, E> }
  >();
