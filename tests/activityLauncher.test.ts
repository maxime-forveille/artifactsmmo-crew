...
const buildDependencies = () => ({
  ...
  execute: vi.fn(
    (_activity: ExecutableActivity): ResultAsync<void, TestActivityError> => okAsync(undefined),
  ),
...
describe("launchActivity", () => {
  ...
  it("returns a Blocker without retrying the Activity", async () => {
    ...
    const result = launchActivity(buildState(), buildAssignment(), dependencies);
  ...
  it("emits Cancellation before starting an aborted Activity", async () => {
    ...
    const result = launchActivity(buildState(), buildAssignment(), dependencies, controller.signal);
    const outcome = await result._unsafeUnwrap().completion;
