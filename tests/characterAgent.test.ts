...
type BankGoldResponse = components["schemas"]["BankGoldTransactionResponseSchema"];
type BankItemResponse = components["schemas"]["BankItemTransactionResponseSchema"];
type CharacterRestResponse = components["schemas"]["CharacterRestResponseSchema"];
...
const expectActionLogged = (character: string, actionName: string): void => {
  expect(loggerInfoMock).toHaveBeenCalledWith({ character }, `${character}: ${actionName}`);
};
...
describe("createCharacterAgent", () => {
  ...
  it("rest forwards the character and refreshes its snapshot", async () => {
    const rest = vi.fn(() => okAsync(buildRestResponse("2024-01-01T00:00:05.000Z")));
  ...
  it("gather forwards the character and refreshes its snapshot", async () => {
    const gather = vi.fn(() => okAsync(buildCraftResponse("2024-01-01T00:00:05.000Z")));
  ...
  it("depositItems forwards items and refreshes its snapshot", async () => {
    const depositItems = vi.fn(() => okAsync(buildBankItemResponse("2024-01-01T00:00:05.000Z")));
  ...
  it("withdrawItems forwards items and refreshes its snapshot", async () => {
    const withdrawItems = vi.fn(() => okAsync(buildBankItemResponse("2024-01-01T00:00:05.000Z")));
    const dependencies: Dependencies = { ...defaultDependencies, withdrawItems };
    const agent = (await createCharacterAgent(dependencies, "Cartman"))._unsafeUnwrap();
    const items = [{ code: "copper_ore", quantity: 3 }];
  ...
  it("depositGold forwards the quantity and refreshes its snapshot", async () => {
    const depositGold = vi.fn(() => okAsync(buildBankGoldResponse("2024-01-01T00:00:05.000Z")));
  ...
  it("withdrawGold forwards the quantity and refreshes its snapshot", async () => {
    const withdrawGold = vi.fn(() => okAsync(buildBankGoldResponse("2024-01-01T00:00:05.000Z")));
  ...
  it("keeps the previous snapshot when a fight response omits this character", async () => {
    ...
    const fight = vi.fn(() =>
      okAsync(
        buildFightResponse("2024-01-01T00:00:05.000Z", [buildCharacter({ hp: 42, name: "Kyle" })]),
      ),
    );
    const dependencies: Dependencies = { ...defaultDependencies, fight };
