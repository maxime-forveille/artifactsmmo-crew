...
describe("parseTaskAssignments", () => {
  it("parses each task type into a character/task assignment", () => {
    const assignments = parseTaskAssignments(
      JSON.stringify({
        ...
        Stan: { items: ["wooden_staff"], monster: "yellow_slime", type: "craftAndEquipThenHunt" },
      }),
    );

    expect(assignments).toHaveLength(5);
    expect(assignments).toContainEqual({ character: "Cartman", task: { type: "autoHunt" } });
  ...
  it("throws a readable error for an unknown task type", () => {
    expect(() => parseTaskAssignments(JSON.stringify({ Cartman: { type: "sleep" } }))).toThrow(
      /Invalid task assignments/,
    );
  });

  it("throws a readable error when a task is missing a required field", () => {
    expect(() => parseTaskAssignments(JSON.stringify({ Cartman: { type: "farm" } }))).toThrow(
      /Invalid task assignments/,
    );
  });
