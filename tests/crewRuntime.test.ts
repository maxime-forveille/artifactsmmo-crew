...
const buildOptions = (
  plan: RollingActivityPlanner<ActivityExecutionError, TestPlanError>,
) => ({
...
describe('createCrewRuntime', () => {
  beforeEach(() => {
    ...
    createCharacterAgentFromSnapshotMock.mockImplementation(
      (_client, character: Character) => ({ name: character.name }),
    );
  ...
  it('seeds Character Agents from the initial Crew Snapshot without extra reads', async () => {
    ...
    const { client, getBankItems, getCharacter, getMyCharacters } = buildClient(
      [stan],
    );
    const options = buildOptions((_snapshot, state) =>
      ok({ activities: [], state }),
    );
    ...
    expect(createCharacterAgentFromSnapshotMock).toHaveBeenCalledWith(
      client,
      stan,
    );
  ...
  it('dispatches a planned Activity through the matching Character Agent', async () => {
    ...
    const plan = vi.fn(
      (_snapshot: CrewSnapshot, state: OrchestratorState, outcome?: unknown) =>
    ...
    expect(runActivityMock).toHaveBeenCalledWith(
      client,
      { name: 'Stan' },
      assignment.activity,
    );
  ...
  it('reports and retries a transient Activity execution failure', async () => {
    ...
    runActivityMock
      .mockReturnValueOnce(errAsync(apiError))
