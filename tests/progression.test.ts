...
const buildFightLog = (
  ...
  ({
    content: {
      fight: { characters: [{ character_name: 'Cartman', xp }], opponent },
    },
...
describe('findNextSafeMonster', () => {
  it("picks the highest-level monster that's still safe when there's no fight history", async () => {
    ...
    const safeAndStronger = buildMonster({
      code: 'yellow_slime',
      hp: 70,
      level: 2,
    });
    const getMonsters = vi.fn(() =>
      okAsync(buildMonsterPage([weak, safeAndStronger])),
    );
  ...
  it('prefers the safe monster with the best observed XP/second rate over a higher level one', async () => {
    ...
    const slowerButHigherLevel = buildMonster({
      code: 'yellow_slime',
      hp: 70,
      level: 2,
    });
    const getMonsters = vi.fn(() =>
      okAsync(buildMonsterPage([fastXp, slowerButHigherLevel])),
    );
  ...
  it('falls back to the highest-level heuristic for monsters with no observed rate yet', async () => {
    ...
    const getCharacterLogs = vi.fn(() =>
      okAsync(buildLogPage([buildFightLog('cow', 50, 40)])),
    );
  ...
  it("skips monsters that aren't safe, even if higher level", async () => {
    ...
    const result = await findNextSafeMonster(
      { getCharacterLogs: noLogs, getMonsters },
      character,
    );
...
describe('findNextFarmableResource', () => {
  it("picks the highest-level resource at or below the character's skill level", async () => {
    ...
    const low = buildResource({
      code: 'copper_rocks',
      level: 1,
      skill: 'mining',
    });
    const high = buildResource({
      code: 'iron_rocks',
      level: 8,
      skill: 'mining',
    });
    ...
    const result = await findNextFarmableResource(
      { getResources },
      character,
      'mining',
    );
