...
const buildGetMaps = () =>
  vi.fn((query: MapQuery = {}) =>
    okAsync(buildPage([buildMap(query.content_type === "bank" ? BANK_MAP_ID : MONSTER_MAP_ID)])),
  );
...
const buildFightResult = (character: CharacterSnapshot) => ({
  ...
  fight: { characters: [], logs: [], opponent: "chicken", result: "win" as const, turns: 3 },
});
...
describe("runHuntingCycle", () => {
  it("moves to the monster, fights until full, then moves to the bank and deposits everything", async () => {
    ...
    expect(depositItems).toHaveBeenCalledWith([{ code: "feather", quantity: 20 }]);
  });
