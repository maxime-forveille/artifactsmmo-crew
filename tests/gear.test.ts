...
describe("findBestCombatGear", () => {
  describe("weapon slot", () => {
    ...
    it("still ranks candidates by damage output when the monster can never deal damage back", async () => {
      ...
      const getItem = vi.fn();
      const getItems = vi.fn(() => okAsync(buildItemPage([weakWeapon, strongWeapon])));
  ...
  describe("helmet slot", () => {
    it("picks the helmet with the best hp/damage contribution", async () => {
      ...
      const getItem = vi.fn();
      const getItems = vi.fn(() => okAsync(buildItemPage([basicHelmet, betterHelmet])));
      ...
      expect(getItems).toHaveBeenCalledWith({ max_level: 5, size: 100, type: "helmet" });
    ...
    it("removes the currently equipped helmet's own contribution before comparing candidates", async () => {
      const character = buildCharacter({ attack_earth: 10, helmet_slot: "old_helmet", hp: 110 });
      const monster = buildMonster({ attack_earth: 5 });
      ...
      const getItems = vi.fn(() => okAsync(buildItemPage([oldHelmet, newHelmet])));
