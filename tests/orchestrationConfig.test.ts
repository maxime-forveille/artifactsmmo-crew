...
describe("parseOrchestrationConfig", () => {
  ...
  it.each(["id", "itemCode", "resourceCode"])("rejects an empty %s", (field) => {
    expect(() =>
      parseOrchestrationConfig(
        buildRawConfig({
          goals: [
  ...
  it("reports a root path when the whole configuration has the wrong shape", () => {
    expect(() => parseOrchestrationConfig(JSON.stringify(null))).toThrow(/\(root\)/);
  });
  ...
  it("accepts an empty Goal list", () => {
    expect(parseOrchestrationConfig(JSON.stringify({ goals: [] }))).toEqual({ goals: [] });
  });
});

describe("loadOrchestrationConfig", () => {
  it("reads and validates an explicit configuration path", () => {
    const directory = mkdtempSync(join(tmpdir(), "artifactsmmo-orchestration-"));
    const path = join(directory, "crew.json");
    writeFileSync(path, buildRawConfig(), "utf-8");

    try {
      expect(loadOrchestrationConfig(path)).toEqual(parseOrchestrationConfig(buildRawConfig()));
    } finally {
